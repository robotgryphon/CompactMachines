...
//import dev.compactmods.machines.neoforge.upgrade.graph.RoomUpgradeConnection;
//import dev.compactmods.machines.neoforge.upgrade.graph.UpgradeConnectionEntry;
...
//                    .getOrThrow(true, LoggingUtil.modLog()::error);
//
//            upgrades.forEach(conn -> {
//                final var room = conn.room();
//                final var data = conn.instance();
//                if (!inst.addUpgrade(data, room))
//                    LoggingUtil.modLog().warn("Failed to load room upgrade for room {}: {}", room, conn.upgradeKey().toString());
//            });
//        }
//
//        return inst;
