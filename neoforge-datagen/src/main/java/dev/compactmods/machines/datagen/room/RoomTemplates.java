...
public class RoomTemplates {
    ...
    public static void make(GatherDataEvent event) {
        ...
        gen.addProvider(event.includeServer(), (DataProvider.Factory<DatapackBuiltinEntriesProvider>) output -> new DatapackBuiltinEntriesProvider(output,
                event.getLookupProvider(),
                new RegistrySetBuilder().add(Rooms.TEMPLATE_REG_KEY, ctx -> {
                    ctx.register(ResourceKey.create(Rooms.TEMPLATE_REG_KEY, new ResourceLocation(Constants.MOD_ID, "tiny")), LegacySizedTemplates.EMPTY_TINY.template());
